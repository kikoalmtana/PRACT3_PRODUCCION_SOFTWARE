...
def test_create_multiple_expenses_and_list():
    ...
    titles = [expense.title for expense in service.list_expenses()]
...
def test_remove_expense_reduces_total():
    ...
    service.create_expense("Libro", 15, "Librería", date(2025, 1, 10))
    service.create_expense("Revista", 4, "Librería", date(2025, 1, 20))

    gastos_actuales = service.list_expenses()

    service.remove_expense(gastos_actuales[0].id)

    assert len(service.list_expenses()) != len(gastos_actuales)
...
def test_update_expense_partial_fields():
    ...
    camiseta = service.create_expense("Camiseta", 15, "Ropa", date(2025, 1, 10))

    service.update_expense(camiseta.id, None, 18, None)

    camiseta = service.list_expenses()[0]

    assert camiseta.title == "Camiseta"
    assert camiseta.amount == 18
    assert camiseta.description == "Ropa"
...
def test_total_amount_after_removal():
    ...
    service.create_expense("Cursos", 30, "Estudios", date(2025, 1, 10))
    service.create_expense("Internet", 25, "Internet", date(2025, 1, 10))
